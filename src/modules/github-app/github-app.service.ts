...
export class GithubAppService {
    constructor(
        ...
        private githubApp: Model<GithubAppDocument>,
        @InjectModel(Repository.name)
        private repository: Model<RepositoryDocument>,
    ) {}
    ...
    public async createInstallationToken(installationId: string) {
        try {
            ...
            console.log(
                `Installation access token ${githubInstallationAccessToken}`,
            );
        ...
        for (let i = 0; i < githubApps.length; i += 1) {
            ...
            try {
                ...
                if (response.status === 204) {
                    console.log(
                        `Uninstalled app ${githubApps[i].installationId}`,
                    );
                } else {
                    console.log(
                ...
                // console.log(error);
                console.log(
                    `Error uninstalling app ${githubApps[i].installationId}`,
                );
            }
        }
        ...
        if (event === 'installation_repositories') {
            ...
            if (data.action === 'added' && 'repositories_added' in data) {
                ...
                for (let i = 0; i < data.repositories_added.length; i += 1) {
                    addedRepos.push(
                        data.repositories_added[i].full_name.toString(),
                    );
                }
