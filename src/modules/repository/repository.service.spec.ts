...
const setupService = async (overrides = {}) => {
  ...
  const module: TestingModule = await Test.createTestingModule({
    providers: [
      ...
      { provide: getModelToken(GithubApp.name), useValue: mockGithubAppModel },
    ],
  }).compile();
...
const setupUserMock = (userModel, userExists = true) => {
  userModel.findById.mockReturnValue({
    exec: jest.fn().mockResolvedValue(userExists ? { id: MOCK_USER_ID } : null),
  });
};
...
describe('RepositoryService', () => {
  ...
  describe('getAllRepos', () => {
    ...
    it('should throw UnauthorizedException when user is not found', async () => {
      ...
      await expect(setup.service.getAllRepos(MOCK_USER_ID)).rejects.toThrow(
        UnauthorizedException,
      );
    });
    ...
    it('should fetch and return repositories successfully', async () => {
      ...
      setup.repositoryModel.bulkWrite.mockResolvedValue({ acknowledged: true });
      ...
      const result = await setup.service.getAllRepos(MOCK_USER_ID);
    ...
    it('should handle errors during repository fetch', async () => {
      ...
      const result = await setup.service.getAllRepos(MOCK_USER_ID);
      expect(result).toEqual({ repositories: [], totalCount: 0 });
    });
  });
  ...
  describe('selectRepos', () => {
    it('should throw NotFoundException when repositories are not found', async () => {
      ...
      await expect(setup.service.selectRepos(['id1', 'id2'])).rejects.toThrow(
        NotFoundException,
      );
    });

    it('should update repositories as selected', async () => {
      const urlIds = ['67ce9701e6bc4af43b0ea01b', '67ce9709e6bc4af43b0ea01c'];
      const mockRepos = urlIds.map((id) => ({ _id: new Types.ObjectId(id) }));
      setup.repositoryModel.find.mockResolvedValue(mockRepos);
      setup.repositoryModel.updateMany.mockResolvedValue({ modifiedCount: 2 });
  ...
  describe('selectRepo', () => {
    ...
    it('should update repository as selected', async () => {
      ...
      setup.repositoryModel.updateOne.mockResolvedValue({ modifiedCount: 1 });
  ...
  describe('selectedRepos', () => {
    ...
    it('should return selected repositories', async () => {
      ...
      const result = await setup.service.selectedRepos(1, 10, MOCK_USER_ID);
      expect(result).toEqual({
        repositories: [{ id: 'repo1', isSelected: true }],
        totalCount: 1,
      });
  ...
  describe('selectAll', () => {
    it('should update all repositories as selected', async () => {
      setup.repositoryModel.updateMany.mockResolvedValue({ modifiedCount: 5 });
