...
export class AuthService {
  ...
  async signup(dto: SignupDto) {
    ...
    this.emailService.sendEmail(dto.email);
  ...
  async login(dto: LoginDto) {
    const user = await this.userModel
      .findOne({ email: dto.email, loginType: 'credential' })
      .exec();
  ...
  async refreshTokens(refreshToken: string) {
    ...
    const tokens = this.generateTokens(user.id, user.email); // Use _id for Mongoose
