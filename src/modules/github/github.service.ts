...
import { GithubUrl, GithubUrlDocument } from 'src/schemas/github-url.schema';
...
export class GithubService {
  constructor(
    ...
    @InjectModel(GithubUrl.name)
    private githubUrlModel: Model<GithubUrlDocument>,
  ) {}
  ...
  async getAllRepos(userId: string) {
    ...
    try {
      ...
      await this.githubUrlModel.bulkWrite(bulkOps);

      // Fetch and return all saved URLs for the user
      const savedUrls = await this.githubUrlModel
        .find({ user: user._id })
        .exec();
  ...
  async selectRepos(urlIds: string[]) {
    const updated = await this.githubUrlModel.updateMany(
  ...
  async readRepo(repoId: string) {
    ...
    const repo = await this.githubUrlModel.findById(repoId);
    if (!repo) {
  ...
  public async createAppAccessToken(installationId: string) {
    try {
